...
public:
    ...
    void moveCursorUp();
    void moveCursorDown();
    void undoCursorUpDown(unsigned int prevCursorColumn, unsigned int prevCursorLine);

    void moveCursorHome();
    void moveCursorEnd();
    void undoMoveCursorHomeEnd(int prevColumn);

    void typeIntoModel(char c);
    void undoTyping(std::vector<std::string>& prevStrVec, 
        std::vector<unsigned int>& prevColVec, unsigned int& prevCursorColumn);

    void createNewLine();
    void undoNewLine(unsigned int& prevCursorCol, 
        std::vector<std::string>& prevStrVec, 
        std::vector<unsigned int>& prevIntVec);

    void deleteLine();
    void undoDeleteLine(unsigned int& prevCursorCol, 
    ...
    void undoDeleteCharacter(unsigned int prevCursorColumn, unsigned int prevCursorLine, unsigned int prevLineAmount,
    std::vector<std::string> prevStrVector, std::vector<unsigned int> prevIntVector);


    std::vector<std::string> getStringVector();
    std::vector<unsigned int> getIntVector();
...
private:
    ...
    unsigned int currentCursorLine;
    unsigned int currentCursorColumn;

    unsigned int LineAmount;

    std::string CurrentErrorMessage;

    std::vector<unsigned int> vectorOfColumns;
